...
path = os.path.join(os.path.dirname(os.getcwd()),  'SimulationEngine', 'output', '2021-11-23', '0')
print(path)
dlist = []
...
data = []
trace0 = dict(
    lat=df.loc[df['step']==1,'y'],
    lon=df.loc[df['step']==1,'x'],
    type="scattermapbox",
    mode='markers',
    marker=dict(
        size=6,
        color= list(map(lambda x:  'blue' if x == 'susceptible' else ('orange' if x == 'exposed' else ('purple' if x == 'asymptomatic' else ('olive' if x == 'vaccinated' else ('olive' if x == 'boosted' else ('green' if x == 'recovered' else ('black' if x == 'dead' else 'red')))))), df.loc[df['step']==1,'state'].values)),
        opacity=1,
        colorscale="jet"),
    text=df.loc[df['step']==1,'state'].values,
)
data = [trace0]
