...
def plot(df):
    ...
    fig = make_subplots(rows=1, cols=3, subplot_titles=sub_groups, vertical_spacing=0.01, horizontal_spacing=0.01,
                        shared_yaxes=True)
...
def plot_age(df):
    ...
    fig.update_layout(showlegend=True, autosize=True, width=1500, height=1000,
                      legend=dict(orientation="h", x=0, y=-0.5, traceorder="normal"),
                      font=dict(family="Arial", size=12))
...
def plot_race(df):
    ...
    fig.update_xaxes(showline=True, linewidth=1, linecolor='black', mirror=True, ticklabelmode="period", dtick="M1")
    fig.update_yaxes(showline=True, linewidth=1, linecolor='black', mirror=True, range=[0, 1000])
    fig.update_layout(showlegend=True, autosize=True, width=1500, height=1800,
                      legend=dict(orientation="h", x=0, y=-0.5, traceorder="normal"),
                      font=dict(family="Arial", size=12))
...
mode = 'ByFPL' # 'All', 'ByAge', 'ByRace', 'ByFPL'
...
df = pd.concat(dlist)
pop = pd.read_csv(os.path.join(os.path.dirname(os.getcwd()), 'SimulationEngine','input', 'persons.csv'))
df = pd.merge(df, pop, on='pid')

if mode=='ByFPL':
    households = pd.read_csv(
        os.path.join(os.path.dirname(os.getcwd()), 'SimulationEngine','input', 'households3.csv'))
...
for step in steps:
    ...
    if mode == 'All':
        dic['cases'] = len(df.loc[(df['step'] == step) & ((df['state'] == 'presymptomatic') | (df['state'] == 'mild'))])
        dic['admissions'] = len(
            df.loc[(df['step'] == step) & ((df['state'] == 'severe') | (df['state'] == 'critical'))])
        dic['deaths'] = len(df.loc[(df['step'] == step) & (df['state'] == 'dead')])
